...
$(document).on('ready', function() {
  ...
  if (top_menu.length) {
    ...
    var topx = (x + menu_bar.height() - $(window).scrollTop());
    //alert(topx);
    if (topx <= 0) {
    ...
    $(document).on('scroll', function() {
      var y = (x + menu_bar.height() - $(window).scrollTop());
      if (y <= 0) {
        menu_bar.fadeIn().addClass('fixed-header').css({
          "background-color": "#ffffff",
          "color": "#333333",
        ...
        submitHandler: function(form, ev) {
          ...
          $.ajax({
            ...
            success: function(data) {
              if (data) {
                $('.sucessMessage').html('Thank you for contacting us! We\ll get back to your shortly!');
                $('.sucessMessage').show();
